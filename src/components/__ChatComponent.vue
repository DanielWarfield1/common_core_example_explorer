    ...
    async handleEnterKey() {
      if (this.searchQuery.trim() !== '') {
        try {
          ...
          // If a response is found, emit the search result, otherwise emit an empty array
          if (response) {
            console.log(response)
            this.$emit('search', response); // Emit the array of responses
          } else {
            console.warn('No response found for query:', this.searchQuery);
          ...
          this.isLoading = false; // Always hide the spinner in case of success or failure
